...
export function removeQueryString(url) {
  ...
  if (queryStringMarkerPos > -1) url = url.substr(0, queryStringMarkerPos);
  return url;
}
export function getQueryString(url) {
  if (url.indexOf("?") > -1) return url.substring(url.indexOf("?"), url.length);
  return undefined;
}
...
export const decomposePath = (url) => {
  ...
  if (queryString != undefined) params = queryStringParser.parse(queryString);
  ...
  if (!isAPI) {
    if (urlParts[2] != undefined && urlParts[2] != "") action = urlParts[2];
    else action = "index";
